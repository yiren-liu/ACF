...
def softmask(x,mask):
    ...
    xxx = T.switch(mask, xx, 0) #if there is no photo in one period, then it will occur nan
    return xxx
...
class AttentionLayer_Item(object):
    def __init__(self,rng, uemb_vec, new_Vemb, n_wordin, n_out,mask,name,prefix=None):
        ...
        items_emb = new_Vemb     # [batch_size,num_video,128]
        attenu = T.dot(self.inputu, self.Wu_I).dimshuffle(0, 'x', 1)#get batch_size*1*128
        atten = T.nnet.sigmoid(T.dot(items_emb, self.Wv_I) + attenu + self.b_I)  #relu [batch_size,num_video,128]
        atten = T.sum(atten * self.c_I, axis=2, dtype='float32')     #get [batch_size,num_video]
        atten = softmask(atten,mask)#get [batch_size,num_video]
        output = atten.dimshuffle(0,1,'x') * items_emb #get [batch_size,num_video]
        self.output = T.sum(output, axis=1, dtype='float32')
